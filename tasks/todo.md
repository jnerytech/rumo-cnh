...
- [x] **Tarefa:** Vite 6 + React 19 + TS strict + Vitest + ESLint/Prettier, com todos os scripts do mapa
...
- [x] **Tarefa:** `scripts/cobertura-placas.ts` + geração de `src/placas/acervo.ts`
...
- [x] **Tarefa:** `QuestaoFonte`, `Dificuldade`, `carregarQuestoes()`
...
- [x] **Tarefa:** baixar `R-28 R-7 R-6a R-5a A-31 A-15 R-25c R-38 A-14 R-32 R-37 A-2b` para `public/placas/`
...
- [x] **Tarefa:** `embaralhar` (Fisher–Yates, RNG injetável) + `prepararQuestao` → `QuestaoPreparada`
...
- [x] **Tarefa:** trocar Fisher–Yates por `sort(() => Math.random() - 0.5)` temporariamente e confirmar que o teste **falha**; reverter
...
- [x] **Tarefa:** `scripts/contact-sheet.ts` → HTML com todas as placas baixadas, cada uma sob seu código
...
- [x] **Tarefa:** `filtrar` (com `temPlaca` injetado) + `sortearSimulado`
...
## B5 · Componente `<Placa/>`
- [x] **Tarefa:** `<Placa codigo="R-28" />` — `<img>` com `alt="Placa R-28"`, `null` se fora do acervo
...
- [x] **Tarefa:** levar `src/dados/` a 100% de branches e travar o limite na config do Vitest
...
- [x] **Tarefa:** completar o acervo em ordem de frequência; desenhar à mão o que o acervo externo não tiver
...
## Onda 1 fechada — 2026-08-27

Todas as 11 tarefas entregues, cada uma com commit próprio. Resultado apurado:

| Critério | Alvo | Real |
|---|---|---|
| Cobertura de branches em `src/dados/` | 100% | **100%** (travado no `vite.config.ts`) |
| Testes | — | **47** |
| Acervo de placas | 69/69 | **69/69** |
| Questões visuais respondíveis | 171/171 | **171/171** |
| Build de produção | limpo | limpo |

Marco 2 alcançado direto, sem parar no Marco 1: o acervo do Commons cobriu 66
dos 69 códigos, e os 3 restantes (R-44, R-35, SAU-06) foram desenhados.

Pendente de conferência humana: as 57 placas novas no contact sheet, com atenção
especial às 3 desenhadas à mão.

Próximo: `SPEC-modo-estudo.md`.

---

# Tarefas — onda 2 (`modo-estudo`)
